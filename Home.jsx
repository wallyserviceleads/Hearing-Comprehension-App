...
const AuditoryTrainer = () => {
  ...
  const generateNoise = (duration, noiseLevel, type) => {
    ...
    let tonalConfig = null;

    switch (type) {
      case 'restaurant':
        tonalConfig = {
          components: [
            { frequency: 180, amplitude: 0.6 },
            { frequency: 320, amplitude: 0.4 },
            { frequency: 520, amplitude: 0.35 }
          ],
          levelScale: 0.3,
          textureScale: 0.08
        };
      ...
      case 'tv':
        tonalConfig = {
          components: [
            { frequency: 260, amplitude: 0.5 },
            { frequency: 520, amplitude: 0.35 },
            { frequency: 1040, amplitude: 0.2 }
          ],
          levelScale: 0.4,
          textureScale: 0.07
        };
      ...
      case 'traffic':
        tonalConfig = {
          components: [
            { frequency: 60, amplitude: 0.8 },
            { frequency: 120, amplitude: 0.6 },
            { frequency: 240, amplitude: 0.4 }
          ],
          levelScale: 0.35,
          textureScale: 0.06
        };
      ...
      case 'crowd':
        tonalConfig = {
          components: [
            { frequency: 140, amplitude: 0.5 },
            { frequency: 220, amplitude: 0.45 },
            { frequency: 340, amplitude: 0.35 },
            { frequency: 460, amplitude: 0.25 }
          ],
          levelScale: 0.25,
          textureScale: 0.09
        };
      ...
      default:
        tonalConfig = {
          components: [{ frequency: 200, amplitude: 0.5 }],
          levelScale: 0.3,
          textureScale: 0.05
        };
    }

    const phases = tonalConfig.components.map(() => Math.random() * 2 * Math.PI);

    for (let i = 0; i < bufferSize; i++) {
      const t = i / sampleRate;
      let tonalSample = 0;

      tonalConfig.components.forEach((component, index) => {
        tonalSample += Math.sin(2 * Math.PI * component.frequency * t + phases[index]) * component.amplitude;
      });

      const texturedSample = tonalSample * tonalConfig.levelScale + (Math.random() * 2 - 1) * tonalConfig.textureScale;
      output[i] = texturedSample * noiseLevel;
    }
